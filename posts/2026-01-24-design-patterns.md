...
내 경우 판매 원장 도메인에 팩토리 메소드 패턴을 적용했다.
해당 영역은 서로 다른 5가지 원장 타입이 존재했고, 비즈니스 상황에 따라 원장이 추가되거나 제거될 가능성이 높은 구조였다.

이러한 변동성을 고려하여,
계정계에서 원장을 조회하는 로직을 원장별 구현체로 분리하고 팩토리 메소드를 통해 객체 생성을 위임함으로써
__변경에는 닫혀 있고(Open/Closed Principle), 확장에는 열려 있는 구조로 설계했다.__
...
새삼 느끼는 건, 디자인 패턴들이 꽤 비슷한 구조로 묶인다는 점이다. 팩토리, 전략, 템플릿 메서드가 한 묶음이고, 프록시, 전략, 데코레이터도 유사한 형태를 띄고 있다.
왜 그럴까?

프록시는 객체의 __대리자 역할(그 자체)__ 을 의도하고, 전략 패턴은 그 대리자를 **교체**하는 데 초점을 두며, 데코레이터는 대리자를 통해 **객체를 장식**하면서 실제 객체에 접근한다.
결국 세 패턴 모두 인터페이스를 통해 상호작용하는 객체 사이에 틈을 만들고,
각자의 의도에 맞게 동작하도록 설계하는 방법들이다.
전략 패턴이 왜 그토록 강조되었는지, 이제서야 조금씩 감이 오는 것 같다.

아래 예시 코드를 통해 전에 판매원장에 적용하면 좋겠다고 판단이 되는 부분을 복합패턴만으로 구현해보았는데,
...
복합 패턴만으로도 내가 의도한 문제를 충분히 풀 수 있었을 것 같다.

팩토리가 불필요하다고 생각한 이유는, 내 경우 팩토리를 통해 생성된 객체가 다른 서비스에서 재사용되지 않았기 때문이다. 팩토리 패턴의 핵심 이점은 객체 생성을 위임하면서 동시에 클라이언트로부터 생성 로직을 숨기는 것인데,
나는 이미 Spring이 객체 생성을 담당하고 있었고 팩토리는 단순한 분기 처리에 그치고 있었다.

반면 복합 패턴과 stream().filter를 조합하면 같은 문제를 훨씬 단순하게 해결할 수 있다.
비즈니스 로직은 그대로 유지하면서 확장에는 열려 있는 구조가 되고, 팩토리를 도입했을 때 생기는 불필요한 클래스 파일과 switch문을 제거할 수 있어 OCP 측면에서도 더 나은 구조가 됐을 것 같다.
